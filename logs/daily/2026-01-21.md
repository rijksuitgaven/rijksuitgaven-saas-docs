...
- **Export method:** PhpMyAdmin → CSV
- **Table rename:** `stad` → `gemeente` (only change, all fields stay the same)
- **Tables to export:** Source tables only (7 total), skip all pivot tables
...
**Blockers:** None
**Next Action:** Data migration (PhpMyAdmin CSV export → Supabase import)
**Data Migration Prep:** Export ready via PhpMyAdmin, only rename is `stad` → `gemeente`
